          ...
          >
            ...
            >
              ...
              <div class="book-title-author-wrapper">
                <div class="book-author book-title">{{book.author}}</div>
                <div class="book-author book-title">{{book.categoryText}}</div>
              </div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      // 通过行数和列数计算更改data的数据结构，实现将一维数组分行的效果
      bookData () {
        ...
        if (data && data.length > 0) {
          ...
          const _bookData = data.slice(0, number) // 对data进行截断
          const _bookDataRow = [] // 最终生成数组
          let _row = 0
          while (_row < row) {
